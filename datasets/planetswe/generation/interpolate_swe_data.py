...
def earthswe_to_well(in_path, out_path):
    ...
    with h5.File(out_path, 'w') as new_file:
        ...
        time = new_file['dimensions'].create_dataset('time', data=orig_file['scales']['sim_time'], dtype='f4')
        time.attrs['sample_varying'] = False
        # Same coordinates for x and y in this specific data
        print(orig_file['scales'].keys())
        d = new_file['dimensions'].create_dataset('phi', data=orig_file['scales']['phi_hash_7b8ec7cabc40ac4b596a5ef833e9eab019f07d46'], dtype='f4')
        d.attrs['time_varying'] = False
        d.attrs['sample_varying'] = False
        d = new_file['dimensions'].create_dataset('theta', data=orig_file['scales']['theta_hash_47f1a1c5acad69381fef2149e23fb804716211f6'], dtype='f4')
        d.attrs['time_varying'] = False
        d.attrs['sample_varying'] = False

        h, u = dedalus_interpolate(orig_file['tasks']['h'][:], orig_file['tasks']['u'][:])
        # T0 Data        
        new_file['t0_fields'].attrs['field_names'] = ['height']
        f = new_file['t0_fields'].create_dataset('height', 
                                                 data=np.transpose(h[np.newaxis, ...], (0, 1, 3, 2)), 
                                                                    dtype='f4')
        ...
        f = new_file['t1_fields'].create_dataset('velocity', 
...
def dedalus_interpolate(h, u):
    ...
    for j in range(u.shape[0]):
        ...
        for i, pt in enumerate(
            np.linspace(np.pi - delta / 2, delta / 2, ntheta)
        ):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--source", default="/mnt/home/polymathic/ceph/the_well/testing_before_adding/earthswe")
    parser.add_argument("--dest", default="/mnt/home/polymathic/ceph/the_well/datasets/planetswe/data")
