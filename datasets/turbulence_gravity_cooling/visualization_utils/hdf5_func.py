...
class Units:
    def __init__(self, density):
        ...
        if density == "dense":
            ...
            self.length = UnitValue(1.85417801326507e20, "cm")
            self.velocity = UnitValue(8.46420276462703, "km/s")
            self.mass = UnitValue(1.989e39, "g")  # [g]
            ...
            self.length = UnitValue(3.98876347381883e20, "cm")
            ...
            self.time = UnitValue(2.18643803130574e15, "s")
            self.length = UnitValue(8.59353039832737e20, "cm")
            ...
            self.time = UnitValue(3.085678e16, "s")
            self.length = UnitValue(3.085678e21, "cm")
            self.velocity = UnitValue(1, "km/s")
            self.mass = UnitValue(1.989e43, "g")  # [g]

        # u_pres
        self.pressure = UnitValue(
            self.mass.value / (self.length.value * self.time.value**2), "g/(cm⋅s^2)"
        )
...
class UnitsLoader:
    def __init__(self, density="dense"):
        self.mu = UnitValue(0.6, "")
        self.H_mass = UnitValue(1 / (6.02e23), "g")  # [g]
        self.u_mass = UnitValue(1.989e39, "g")  # [g]
        self.kBoltz = UnitValue(1.380649e-16, "cm^2 g/s^2/K")  # Boltzmann constant
        self.pc = UnitValue(3.0857e18, "cm")  # [cm]
        self.yr = UnitValue(3.15576e7, "s")
        self.Msolar = UnitValue(1.989e33, "g")
