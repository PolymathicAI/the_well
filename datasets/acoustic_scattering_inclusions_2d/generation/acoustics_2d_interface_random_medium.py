...
def setup(kernel_language='Fortran', use_petsc=False, outdir='./_output', 
          ...
          disable_output=False, num_cells=(256, 256), seed=None,
          include_splits=True, include_inclusions=True, T_max=2., num_steps=101):
    """
    ...
    midpoint = seed.uniform(-.8, .8)
    rho_left   = seed.uniform(.2, 7) #4.0  # Density in left half
    rho_right  = seed.uniform(.2, 7) # 1.0  # Density in right half
    bulk_left  = 4.0  # Bulk modulus in left half
    bulk_right = 4.0  # Bulk modulus in right half


    def gaussian_bump(aux, mask, seed, rho_low=1, rho_high=7., sigma_low=.1, sigma_high=5):
        ...
        rho = rho_base + (rho_bump-rho_base)*np.exp(-((Xmask-xc)**2 + (Ymask-yc)**2)/(sigma))
        c = np.sqrt(bulk_left/rho)
        aux[0][mask] = rho
        aux[1][mask] = c

    def linear_gradient(aux, mask, seed, rho_low=1, rho_high=7.):
        ...
        rho = ((1 - Xrel) * (1 - Yrel) * rho_x0 + Xrel * (1 - Yrel) * rho_x1 
               + (1 - Xrel) * Yrel * rho_y0 + Xrel * Yrel * rho_y1)
        c = np.sqrt(bulk_left/rho)
        aux[0][mask] = rho
        aux[1][mask] = c


    def constant(aux, mask, seed, rho_low=1, rho_high=7.):
        rho = seed.uniform(rho_low, rho_high)
        c = np.sqrt(bulk_left/rho)
        aux[0][mask] = rho
        aux[1][mask] = c

    def smoothed_gaussian_noise(aux, mask, seed, rho_low=1, rho_high=7., std=2, sigma_low=5, sigma_high=10):
        rho = seed.uniform(rho_low, rho_high)
    ...
    gen_funcs = [gaussian_bump, linear_gradient, constant, 
                   smoothed_gaussian_noise]
    ...
    for i in range(n_waves):
        center = seed.uniform(-.95, .95, 2)
        x0 = center[0]
        y0 = center[1]
        width = seed.uniform(.05, .15)
        rad = seed.uniform(width+.01, .3)
        intensity = seed.uniform(.5, 2.)
        # x0 = -0.5; y0 = 0.
        r = np.sqrt((X-x0)**2 + (Y-y0)**2)
        # width = 0.1; rad = 0.25
        state.q[0,:,:] += (np.abs(r-rad)<=width)*(intensity+np.cos(np.pi*(r-rad)/width))

    if include_inclusions:
        n_inclusions = seed.integers(0, 15)
        for i in range(n_inclusions):
        # Copied elipse code from 
            g_ell_center = seed.uniform(-.95, .95, 2)
            rads = seed.uniform(.05, .6, 2)
            g_ell_width = rads[0]
            g_ell_height = rads[1]
