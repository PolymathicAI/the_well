...
class GenericWellLoader(Dataset):
    ...
    def __getitem__(self, indices):
        if not isinstance(indices, list):
            indices = [indices] # Sometimes list indexing will be easier
        xs = []
        ys = []
        for index in indices:
            ...
            bcs = [] 
            xs.append(trajectory[:self.n_steps_input])
            ys.append(trajectory[self.n_steps_input:])
        return {'x': np.stack(xs, 0), 'y': np.stack(ys, 0), 'bcs': np.array(bcs)}
    ...
    Note - doesn't currently normalize internally - need to add
