...
class DilatedBlock(nn.Module):
    def __init__(self, in_channels, out_channels, n_spatial_dims,
                  kernel_size, num_levels, padding_type="zero"):
        ...
        for rate in dilation_rates:
            ...
            conv_layers.append(conv(in_channels, out_channels,
                                     kernel_size=kernel_size, dilation=rate,
                                     padding=padding))
...
class DilatedResNet(nn.Module):
    def __init__(self, dim_in: int,
        ...
        if self.padding_type == "zero":
            self.input_conv = conv(dim_in, hidden_dim, kernel_size=kernel_size, padding=1)
        else:
            self.input_conv = conv(dim_in, hidden_dim, kernel_size=kernel_size, padding=0)

        self.processors = nn.ModuleList([DilatedBlock(hidden_dim, hidden_dim,
                                                      self.n_spatial_dims, kernel_size, levels_per_block,
                                                      padding_type) for _ in range(blocks)])

        if self.padding_type == "zero":
            self.output_conv = conv(hidden_dim, dim_out, kernel_size=kernel_size, padding=1)
        else:
            self.output_conv = conv(hidden_dim, dim_out, kernel_size=kernel_size, padding=0)
