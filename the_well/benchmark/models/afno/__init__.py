...
Simplified some unused parameters since they're
...
class ComplexBlockLinear(nn.Module):
    def __init__(self, hidden_dim, bias=True,
        ...
        self.weight = nn.Parameter(torch.view_as_real(
            self.scale * 
                                   torch.randn(cmlp_diagonal_blocks, 
                                               self.block_size, self.block_size, dtype=torch.cfloat))
        )
...
class AFNO_ND(nn.Module):
    def __init__(self, hidden_size: int,
        ...
        super().__init__()
        assert hidden_size % cmlp_diagonal_blocks == 0, f"hidden_size {hidden_size} should be divisble by cmlp_diagonal_blocks {cmlp_diagonal_blocks}"
    ...
    def forward(self, x):
        ...
        x = torch.fft.irfftn(x, s=tuple(self.resolution), dim=spatial_dims, norm="ortho")
        x = x.type(dtype)
        return x 
...
class Block(nn.Module):
    def __init__(
        ...
        self.filter = AFNO_ND(hidden_dim, resolution, cmlp_diagonal_blocks, sparsity_threshold) 
...
class AFNO(nn.Module):
    def __init__(
        ...
        if self.n_spatial_dims == 2:
            self.patch_embed = nn.Conv2d(dim_in, hidden_dim, kernel_size=patch_size, stride=patch_size)
            self.embed_permutation = [ 
                'b h w c -> b c h w',
                'b c h w -> b h w c',       
            ]
            self.patch_debed = nn.ConvTranspose2d(hidden_dim, dim_out, kernel_size=patch_size, stride=patch_size)

        elif self.n_spatial_dims == 3:
            self.patch_embed = nn.Conv3d(dim_in, hidden_dim, kernel_size=patch_size, stride=patch_size)
            self.embed_permutation = [
                'b h w d c -> b c h w d',
                'b c h w d -> b h w d c',
            ]
            self.patch_debed = nn.ConvTranspose3d(hidden_dim, dim_out, kernel_size=patch_size, stride=patch_size)
        self.inner_size = [k // patch_size for k in self.resolution]
        pos_embed_size = [1] + self.inner_size + [hidden_dim]
        self.pos_embed = nn.Parameter(.02*torch.randn(pos_embed_size))
        self.pos_drop = nn.Dropout(p=drop_rate)

        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, n_blocks)]

        self.blocks = nn.ModuleList([
            Block(hidden_dim=hidden_dim, resolution=self.inner_size, 
                  mlp_ratio=mlp_ratio, drop=drop_rate, drop_path=dpr[i], norm_layer=norm_layer,
            cmlp_diagonal_blocks=self.cmlp_diagonal_blocks, sparsity_threshold=sparsity_threshold,) 
