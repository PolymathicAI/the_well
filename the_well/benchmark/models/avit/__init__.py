...
class AxialAttentionBlock(nn.Module):
    def __init__(self, hidden_dim=768, num_heads=12, n_spatial_dims=2, 
    ...
    def forward(self, x):
        ...
        q, k, v = map(lambda t: rearrange(t, self.head_split, he=self.num_heads), (q, k, v))
...
class AViT(nn.Module):
    ...
    def __init__(self, dim_in: int, 
        ...
        self.patch_size = 16
        # Embedding
        pe_size = tuple(int(k/self.patch_size) for k in self.resolution) + (hidden_dim,)
        self.absolute_pe = nn.Parameter(.02 * torch.randn(*pe_size))
        self.embed = hMLP_stem(dim_in=dim_in,
                                hidden_dim=hidden_dim,
                                  n_spatial_dims=dset_metadata.n_spatial_dims)
        self.blocks = nn.ModuleList([AxialAttentionBlock(hidden_dim=hidden_dim,
                                                         num_heads=num_heads,
                                                         n_spatial_dims=dset_metadata.n_spatial_dims,
                                                         drop_path=self.dp[i])
                                     for i in range(processor_blocks)])
        self.debed = hMLP_output(hidden_dim=hidden_dim,
                                  dim_out=dim_out,
                                  n_spatial_dims=dset_metadata.n_spatial_dims)
