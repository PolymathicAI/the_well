"""Distances between full trajectories."""
import numpy as np
import torch
...
class Histogram_W1(Metric):
    ...
    def eval(
        ...
        """
        space_time = (time_dim,) + tuple(range(-meta.n_spatial_dims - 1, -1))
        x = x.permute(space_time)
        ...
        y_sorted = torch.sort(y_flat, dim=-2).values
        return torch.mean(torch.abs(x_sorted - y_sorted), dim=-2)
