...
**Domain scientists**: Miguel Beneitez and Richard Kerswell, DAMTP, University of Cambridge, UK.
...
- LAM: 1 trajectory with 1 snapshot, 512x512 image(simple invariant solution). 
...
Fields available in the data: $u^*, v^*, p^*, c^*_{xx}, c^*_{yy}, c^*_{zz}, c^*_{xy}$
...
- Laminar: simple solution starting from analytical result. 
...
Boundary conditions: no slip conditions for the velocity ($(u^*,v^*)=(0,0)$) at the wall and $\epsilon=0$ at the wall for the eqiation for $\bold{C^*}$
...
Hardware used to generate the data and precision used for generating the data: typically 32 or 64 cores.
